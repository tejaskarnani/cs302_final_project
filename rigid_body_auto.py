...
if len(sys.argv) != 7:
    print(
        "Usage: python3 rigid_body_auto.py [horiz = 0.5 to 5.0] [branching = 1, 2, 3] [stiffness = 10 to 200] train"
    )
    exit(-1)
else:
    horiz = float(sys.argv[1])
    branching = int(sys.argv[2])
    stiffness = int(sys.argv[3])
    amps = [float(x) for x in sys.argv[4].split(',')] if len(sys.argv) > 4 else [0.1] * 5
    phases = [float(x) for x in sys.argv[5].split(',')] if len(sys.argv) > 5 else [0.0, 0.5, 1.0, 1.5, 2.0]
    cmd = sys.argv[6]
# print(robot_id, cmd)

def main():
    
    setup_robot(*robots[0](horiz, branching, stiffness), amps, phases)
