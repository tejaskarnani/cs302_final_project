...
rest_lengths = ti.field(float, shape=(max_springs,))  # Dynamic rest lengths
base_lengths = ti.field(float, shape=(max_springs,))  # Base rest lengths
...
@ti.kernel
def update_actuation(t: ti.i32):
    for i in range(n_springs):
        rest_lengths[i] = base_lengths[i] * (1 + spring_actuation[i] * ti.sin(spring_omega * t * dt + spring_phase[i]))
...
def setup_robot(objects, springs, h_id, amps=None, phases=None):
    global head_id
    head_id = h_id
    ...
    for i in range(n_springs):
        ...
        base_lengths[i] = s[4]  # Store base length
        rest_lengths[i] = s[4]  # Initialize dynamic rest length
        if s[6]:
            spring_actuation[i] = s[6]
        else:
            spring_actuation[i] = default_actuation
        if amps and i < len(amps):
            spring_actuation[i] = amps[i]  # Override with user-defined amplitude
        if phases and i < len(phases):
            spring_phase[i] = phases[i]  # Override with user-defined phase
...
if len(sys.argv) != 3:
    print(
        "Usage: python3 rigid_body.py 0 train"
    )
    exit(-1)
else:
...
def main():
    if robot_id == 0:
        print("To setup your own fork robot, enter the parameters you want.")
        horiz_scale, branching_factor = 0, 0
        invalid_inputs = True
        while invalid_inputs:
            horiz_scale = float(input("Enter a value between 0.5 and 5 for horizontal scale of the top body: "))
            branching_factor = int(input("Enter a value between 1 and 3 for the branching factor (number of primary spring pairs): "))
            stiffness = int(input("Enter a value between 10 and 200 for spring stiffness: "))
            amps = []
            phases = []
            for i in range(5):  # Assuming 5 springs, adjust based on your robot
                amp = float(input(f"Enter amplitude (0.0-0.2) for spring {i+1}: "))
                phase = float(input(f"Enter phase (0.0-2.0) for spring {i+1}: "))
                amps.append(max(0.0, min(0.2, amp)))
                phases.append(max(0.0, min(2.0, phase)))
            invalid_inputs = (horiz_scale <= 0.49 or branching_factor <= 0 or stiffness <= 9) or (horiz_scale > 5.0 or branching_factor > 3 or stiffness > 200)
            if invalid_inputs:
                print("Invalid inputs: Try again.")
        setup_robot(*robots[robot_id](horiz_scale, branching_factor, stiffness), amps, phases)
    else:
